...
- Milestone 6: the portal is deployed at https://vg-task.vercel.app. All six password users reached their own public dashboards. Password-only aggregate reports, wrong-password rejection, session tampering/expiry, rotation, revocation and rate limits are verified. Desktop and 390-pixel report layouts were checked. An explicit function-permission audit found PostgreSQL's default PUBLIC execution grant surviving a schema-specific revoke; a corrective migration removes it globally and restricts existing functions. EXECUTE tests now catch that independently of table permissions. A second live dispatch submitted 262 SMS recipients and received delivery/bounce/unsubscribe updates. Google integration is prepared but remains disabled pending the user's identity and provider setup; earliest start date and notice period are also pending for the private submission draft.
